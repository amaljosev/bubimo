...
class _TimelineViewState extends State<_TimelineView>
  ...
  List<DiaryEntry> _entriesForDay(DateTime day, List<DiaryEntry> all) {
    final norm = _normalizeDate(day);
    return all.where((e) => _normalizeDate(e.date) == norm).toList();
  }

  Set<DateTime> _markedDates(List<DiaryEntry> entries) {
    return entries.map((e) => _normalizeDate(e.date)).toSet();
  }

  bool _dayHasFavorite(DateTime day, List<DiaryEntry> entries) {
    final norm = _normalizeDate(day);
    return entries.any((e) => _normalizeDate(e.date) == norm && e.isFavorite);
  }

  /// First mood emoji found for the day, or '' if none set.
  String _moodForDay(DateTime day, List<DiaryEntry> entries) {
    final norm = _normalizeDate(day);
    for (final e in entries) {
      if (_normalizeDate(e.date) == norm && e.mood != null) {
        return e.mood!.emoji;
      }
    }
    return '';
  ...
  String _dayName(int wd) => _dayNames[wd - 1];

  String _formatEntryTime(DateTime d) {
  ...
  /// Same asset-vs-file convention as the Diary tab's header image.
                  ...
                  SliverAppBar(
                    ...
                    flexibleSpace: LayoutBuilder(
                      builder: (context, constraints) {
                        final expandRatio =
                            ((constraints.maxHeight - 60) / (200 - 60))
                                .clamp(0.0, 1.0);
                        ...
                        return Stack(
                          ...
                          children: [
                            if (bgImagePath != null)
                              Opacity(
                                opacity: expandRatio,
                                child: _headerImage(bgImagePath),
                              ),
                            if (bgImagePath != null)
                              Opacity(
                                opacity: expandRatio,
                      ...
                      child: Padding(
                        ...
                        child: TableCalendar(
                          ...
                          selectedDayPredicate: (d) =>
                              isSameDay(selectedDay, d),
                          onDaySelected: (sel, foc) => _onDaySelected(sel),
                          onPageChanged: (foc) =>
